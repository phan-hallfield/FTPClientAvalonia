...
namespace FTPClientAvalonia.Services
{
	...
	{ 
		...
		public static async Task<IStorageFile?> OpenFileDialogAsync(this object? context, string title)
		{
			if (context == null)
			{
			...
			if (topLevel != null)
			{
                ...
                if (storageFiles.Count > 0)
					return storageFiles.FirstOrDefault();
            }
			return null;
		}
		
		public static async Task<string?> OpenFolderDialogAsync(this object? context, string title)
		{
			if (context == null)
			{
				throw new ArgumentNullException(nameof(context));
			...
			if (topLevel != null)
			{
				...
				if (storageFolder.Count > 0)
					path = storageFolder.Select(s => s.TryGetLocalPath()).FirstOrDefault();
				//storageFolder.Select(s => s.Path).FirstOrDefault().ToString();
